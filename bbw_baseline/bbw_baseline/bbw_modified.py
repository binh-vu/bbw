...
from kgdata.wikidata.models import WDValue, WDEntity
from bbw_baseline.file_cache import (
    EagerSingleFileCache,
    key_jsondump,
    key_unordered_list,
)
...
def get_parallel(a, n):
    ...
    input_4_gnu_parallel = (
        parallel + "--amount ::: " + amount + " ::: --offset  ::: " + offset
    )
...
def random_user_agent(agent="bot4bbw-"):
    ...
    random_agent = "-" + "".join(
        random.choice(letters) for i in range(random.randrange(4, 9))
    )
...
def get_SPARQL_dataframe_item(name, language, url=url_query, ptype=ptype):
    """
    Parameters
    ----------
    name : str
    ...
    query = (
        """SELECT REDUCED ?value ?valueType ?p2 ?item ?itemType ?itemLabel WHERE {
                ?value rdfs:label """
        + '"'
        + name
        + '"@'
...
def get_SPARQL_dataframe_prop(prop, value, url=url_query, ptype=ptype):
    ...
    subquery.extend(
        ...
        wdt:"""
            ...
            + str(ind)
            + ";"
            for ind, (prop, value) in enumerate(zip(prop, value))
        ]
    )
  ...
  OPTIONAL {?wdproperty wikibase:claim ?p2 ;
   ...
   SERVICE wikibase:label { bd:serviceParam wikibase:language "en". }
    ...
    try:
        ...
        for prop in results:
            if (
                "psvalueLabel" in prop
                and prop.get("psvalueLabel").get("value") is not None
            ):
...
def get_wikidata_title(url, url_front=url_front):
    ...
    try:
        ...
        r = requests.get(
            url, params=params, headers={"User-Agent": random_user_agent()}, timeout=5
        ).json()
        title = (
            r.get("entities")
            .get(url.split("/")[-1])
...
def get_wikimedia2wikidata_title(wikimedia_url):
    ...
    try:
        r = requests.get(
            wikimedia_url, headers={"User-Agent": random_user_agent()}, timeout=1
        )
        ...
        if redirect_url:
            ...
            r = requests.get(
                "https://commons.wikimedia.org" + redirect_url,
                headers={"User-Agent": random_user_agent()},
                timeout=1,
            )
            soup = BeautifulSoup(r.content, "html.parser")
...
def get_wikipedia2wikidata_title(wikipedia_title, url_front=url_front):
    ...
    try:
        ...
        if pages.get("-1"):
            ...
            wikidataID = (
                [k for k in pages.values()][0].get("pageprops").get("wikibase_item")
            )
            bestname = get_title(url_front + "/wiki/" + wikidataID).replace(
                " - Wikidata", ""
            )
    except Exception:
        bestname = None
...
def get_searx_bestname(name):
    ...
    try:
        results = requests.get(
            url, data=data, headers={"User-Agent": random_user_agent()}
        ).json()
        ...
        if len(results.get("suggestions")) > 0:
            bestname.extend(
                [
                    k
                    for k in results.get("suggestions")
                    if not re.search("[\uac00-\ud7a3]", k)
                ]
            )
            ...
            best_sugg = difflib.get_close_matches(
                name,
                [
                    k
                    for k in results.get("suggestions")
            ...
            try:
                ...
                results2 = requests.get(
                    url, data=data2, headers={"User-Agent": random_user_agent()}
                ).json()
                if results2:
                    if len(results2.get("infoboxes")) > 0:
                        bestname.extend(
                            [x.get("infobox") for x in results2.get("infoboxes")]
                        )
            except Exception:
                pass
        # Process corrections
        if len(results.get("corrections")) > 0:
            corrections = [
                corr for corr in results.get("corrections") if '"' not in corr
            ]
            if len(corrections) > 0:
                for correction in corrections:
                    try:
                        data3 = {"q": engines + correction, "format": "json"}
                        results3 = requests.get(
                            url, data=data3, headers={"User-Agent": random_user_agent()}
                        ).json()
                        if results3:
                            if len(results3.get("infoboxes")) > 0:
                                bestname.extend(
                                    [
                                        x.get("infobox")
                                        for x in results3.get("infoboxes")
                                    ]
                                )
                    except Exception:
                        pass
        ...
        if len(results.get("results")) > 0:
            for i, result in enumerate(results.get("results")):
                ...
                if (
                    ...
                    and ("Wikidata:SPARQL" not in result.get("title"))
                ):
                    title = get_title(url)
                    if title:
                        bestname.append(
                ...
                raw_match = difflib.get_close_matches(
                    name,
                    [
                        raw_title.replace(" ...", "")
                        .replace(" ?", "")
        ...
        suggestions = list(
            set(difflib.get_close_matches(name, bestname, n=3, cutoff=0.41))
        )
        suggestions = suggestions + [
            get_openrefine_bestname(best) for best in suggestions
        ]
        suggestions = suggestions + [
            get_wikipedia2wikidata_title(best) for best in suggestions
        ]
...
def get_one_class(classes):
    ...
    if len(classes) == 2 and classes[0][1] == classes[1][1]:
        one_class = BBWSearchFn.get_instance().get_common_class(
...
class BBWSearchFn:
    ...
    def setup(
        ...
        get_searx_bestname_outfile = str(
            (self.cache_dir / "get_searx_bestname.pkl").absolute()
        )
        get_openrefine_bestname_outfile = str(
            (self.cache_dir / "get_openrefine_bestname.pkl").absolute()
        )
        get_common_class_outfile = str(
            (self.cache_dir / "get_common_class.pkl").absolute()
        )

        get_SPARQL_dataframe_outfile = str(
            (self.cache_dir / "get_SPARQL_dataframe.pkl").absolute()
        )
        get_SPARQL_dataframe_type_outfile = str(
            (self.cache_dir / "get_SPARQL_dataframe_type.pkl").absolute()
        )

        self.get_searx_bestname = EagerSingleFileCache.cache_func(
            get_searx_bestname_outfile
        )(get_searx_bestname)
        self.get_openrefine_bestname = EagerSingleFileCache.cache_func(
            get_openrefine_bestname_outfile
        )(get_openrefine_bestname)

        self._original_get_common_class = EagerSingleFileCache.cache_func(
            get_common_class_outfile, key_fn=key_unordered_list
        )(get_common_class)
        self._original_get_SPARQL_dataframe = EagerSingleFileCache.cache_func(
            get_SPARQL_dataframe_outfile
        )(self._original_get_SPARQL_dataframe)
        self._original_get_SPARQL_dataframe_type = EagerSingleFileCache.cache_func(
            get_SPARQL_dataframe_type_outfile
        )(self._original_get_SPARQL_dataframe_type)

        if get_SPARQL_dataframe_type2_flag == self.ENABLE_FLAG:
            self.get_SPARQL_dataframe_type2 = self.big_cache_get_SPARQL_dataframe_type2(
                self.cache_dir / "class_instances"
            )
        elif get_SPARQL_dataframe_type2_flag == self.THROW_ERROR_ON_INVOKE_FLAG:
            self.get_SPARQL_dataframe_type2 = self.big_cache_get_SPARQL_dataframe_type2(
                self.cache_dir / "class_instances", self.always_fail
            )
        else:
        ...
        if enable_override:
            ...
            if get_SPARQL_dataframe_type_flag == self.ENABLE_FLAG:
                self.get_SPARQL_dataframe_type = self.wrap_cell_based_function(
                    self._original_get_SPARQL_dataframe_type
                )
            ...
            self.get_SPARQL_dataframe = self.wrap_cell_based_function(
                self._original_get_SPARQL_dataframe
            )
            self.get_SPARQL_dataframe_type = self.wrap_cell_based_function(
                self._original_get_SPARQL_dataframe_type
    ...
    def _original_get_SPARQL_dataframe(
        self, name, language, url=url_query, extra="", ptype=ptype
    ):
        """
        Parameters
        ----------
        ...
        query = (
            ...
            + extra
            + """?itemType ?p1 ?p2 ?value ?valueType ?valueLabel ?psvalueLabel WHERE {
                    ?item ?p1 """
            + '"'
            + name
            + '"'
    ...
    def _original_get_SPARQL_dataframe_type(
        self, name, datatype, language, url=url_query, ptype=ptype
    ):
        name = name.replace('"', '\\"')
        if language:
            lang = language
    ...
    def _original_get_SPARQL_dataframe_type2(
        self, datatype, language, url=url_query, ptype=ptype
    ):
        ...
        if language:
            ...
            lang = "en"
        query = (
            """SELECT REDUCED ?itemLabel WHERE {
    ...
    def _original_lookup(
        self, name_in_data, language, metalookup=True, openrefine=False
    ):
        """
        Parameters
        ----------
    ...
    def _get_SPARQL_dataframe(
        ...
        ents: List[WDEntity] = [
            ...
            for qnode_id in self.table2links[_table_id_].get((_ri_, _ci_), [])
        ]
        if len(ents) == 0:
            return None
    ...
    def _get_SPARQL_dataframe_type(
        self,
        name,
        datatype,
        language,
    ...
    def _lookup(
        ...
        proper_name = ""
        # Search entity using WD SPARQL-endpoint
        WDdf = self.get_SPARQL_dataframe(
            name_in_data, language, _ri_=_ri_, _ci_=_ci_, _table_id_=_table_id_
        )
        if isinstance(WDdf, pd.DataFrame):
            proper_name = name_in_data
            how_matched = "SPARQL"  # This means we have found a mention of 'name_in_data' in Wikidata using single SPARQL-query
    ...
    def _get_common_class(self, classes: List[str]):
        ...
        ents = [
            self.qnodes[WikidataNamespace.get_entity_id(class_id)]
        ...
        if len(commonParents) == 0:
            ...
            output = WikidataNamespace.get_entity_abs_uri(
                min(commonParents, key=lambda x: x[1])[0]
            )
    ...
    def big_cache_get_SPARQL_dataframe_type2(
        self, big_cache_dir: Path, override_fn=None
    ):
    ...
    def extract_datavalue_label(self, value: WDValue, language):
        ...
        if value.is_entity_id(value):
            if value.as_entity_id() not in self.qnodes:
                ...
                if label == "":
                    label = value.as_entity_id()
            return label
        if value.is_globe_coordinate(value):
            return f"Point({value.value['longitude']} {value.value['latitude']})"
        if value.is_quantity(value):
            ...
            if (
                ...
                if timestr[0] == "+":
                    ...
                    if timestr.find("-00-00T00:00:00") != -1:
                        timestr = timestr.replace("-00-00T00:00:00", "-01-01T00:00:00")
            return timestr
        if value.is_mono_lingual_text(value):
    ...
    def test(self):
        ...
        for i in range(len(WDdf)):
            r = WDdf.iloc[i]
            if not r["item"].endswith("Q2399866"):
                continue
            if not r["p2"].endswith("P17"):
                continue
            print(r)
...
def match(WDdf, target_value):
    ...
    if df.empty and not isfloat(target_value):
        ...
        if df.empty:
            approx_matches = difflib.get_close_matches(
                target_value, WDdf.valueLabel.to_list(), n=3, cutoff=0.95
            )
            if len(approx_matches) == 0:
                approx_matches = difflib.get_close_matches(
                    target_value, WDdf.valueLabel.to_list(), n=3, cutoff=0.5
                )
            if len(approx_matches) > 0:
                ...
                if detect_name(target_value):
                    df = WDdf[
                        WDdf.valueLabel.apply(
                            lambda x: all(
                                word in x.lower()
                                for word in detect_name(target_value).lower()
                            )
                        )
                    ]
    ...
    if df.empty and isdate:
        wd_dates = [
            x for x in WDdf.valueLabel.to_list() if re.match(r"^\d{4}-\d{2}-\d{2}", x)
        ]
        target_datetime = date.fromisoformat(target_value)
        approximate_match = min(
            wd_dates, key=lambda x: abs(date.fromisoformat(x[:10]) - target_datetime)
        )
    ...
    if df.empty and isfloat(target_value):
        ...
        all_approximate_matches = [
            x
            for x in wd_floats
            if (abs(float(x) - float(target_value)) <= 0.02 * abs(float(x)))
        ]
...
def preprocessing(filecsv):
    ...
    if len(filecsv.columns) == 1:  # Data augmentation for single-column tables
        filecsv[1] = filecsv[0]
    filecsv = filecsv.applymap(
        lambda x: ftfy.fix_text(x)
    )  # fix encoding and clean text
    return filecsv


def contextual_matching(
    filecsv,
    filename="",
    language="",
    semtab=False,
    ...
    if step2:
        for row in range(
            1, rows
        ):  # We start here from row=1, because there are "col0" and "col1" in row=0
            name_in_data = filecsv.iloc[row, 0]
            [WDdf, how_matched, proper_name] = bbw_search_fn.lookup(
                name_in_data, language, _ri_=row, _ci_=0, _table_id_=filename
            )  # Lookup using the value from the 0-column
            this_row_item = []
            matches_per_row = 0
                ...
                if not WDdf.empty:
                    for col in range(col0, cols):
                        try:
                            ...
                            if semtab:
                                df_prop = df[
                                    (df.p2.str.contains(url))
                                    & (~df.item.str.contains("/statement/"))
                                ]
                            else:
                                df_prop = df
                            properties = [
                                x.replace("/prop/P", "/prop/direct/P").replace(
                                    "/direct-normalized/", "/direct/"
                                )
                                for x in df_prop.p2.to_list()
                            ]
                            properties = list(
                                set(zip(properties, df_prop.item.to_list()))
                            )
                            if len(properties) > 0:
                                matches_per_row += 1
                            ...
                            if "itemType" in df_prop:
                                itemType = list(
                                    set(
                                        [
                                            k
                                            for k in df_prop.itemType.to_list()
                                            if k is not np.nan
                                        ]
                                    )
                                )
                                ...
                                itemType = []
                            df_value = df[
                                (~df.value.str.contains("/statement/"))
                                & (df.value.str.contains(url))
                            ]
                            if not df_value.empty:
                                value, valueType = list(
                                    set(df_value.value.to_list())
                                ), list(
                                    set(
                                        [
                                            k
                                            for k in df_value.valueType.to_list()
                                            if k is not np.nan
                                        ]
                                    )
                                )
            ...
            if len(this_row_item) > 0:
                ...
                for i, cpa_row in enumerate(cpa_list[cpa_row_ind:]):
                    if len(cpa_row[4]) > 0:
                        cpa_list[cpa_row_ind + i][4] = [
                            prop for prop in cpa_row[4] if prop[1] == this_row_item
                        ]
        ...
        for row_prop in cpa_list[cpa_ind:]:
            if len(row_prop[4]) > 0:
                if col_prop.get(row_prop[3]):
                    col_prop[row_prop[3]].extend(
                        [cprop[0].split("/")[-1] for cprop in row_prop[4]]
                    )
                else:
                    col_prop[row_prop[3]] = [
                        cprop[0].split("/")[-1] for cprop in row_prop[4]
        ...
        if len(entity_columns) > 0:
            for nrow in nomatch_row or []:
                try:  # Try to use ALL entity columns AT ONCE and their property-relations to the main column
                    WDdf = get_SPARQL_dataframe_prop(
                        prop=[col_prop[ncol] for ncol in entity_columns],
    ...
    if step4:
        ...
        for row in nomatch_row or []:
            for col in entity_columns or []:
                ...
                if not isfloat(value_to_match) and not re.match(
                    r"^(\d{4})/(\d{2})/(\d{2})$", value_to_match
                ):
                    try:
                        WDitem = get_SPARQL_dataframe_item(value_to_match, language)
                        bestname = difflib.get_close_matches(
                            filecsv.iloc[row, 0],
                            WDitem.itemLabel.to_list(),
                            n=2,
                            cutoff=0.95,
                        )
                        if len(bestname) == 0:
                        ...
                        if len(bestname) > 0:
                            ...
                            properties = [
                                ...
                                for x in WD.p2.to_list()
                            ]
                            properties = list(set(zip(properties, WD.item.to_list())))
                            WD = WD[
                                (~WD.value.str.contains("/statement/"))
                                & (WD.value.str.contains(url))
                            ]
                            if not WD.empty:
    ...
    if step5 or step6:
        ...
        for row_type in cea_list[cea_ind:]:
            if len(row_type[4]) > 0:
                if col_type.get(row_type[2]):
                    col_type[row_type[2]].extend(
                        [etype.split("/")[-1] for etype in row_type[4]]
                    )
                else:
                    col_type[row_type[2]] = [
                        etype.split("/")[-1] for etype in row_type[4]
        ...
        col_type.update(
            (key, [ct[0] for ct in Counter(value).most_common(2)])
            ...
            for ncol in entity_columns or []:
                try:
                    for column_type in col_type[ncol]:
                        WDtype = bbw_search_fn.get_SPARQL_dataframe_type(
                            filecsv.iloc[nrow, ncol], column_type, language
                        )
                        ...
                        if item:
                            cea_list.append(
                                [
                                    ...
                                    ncol,
                                    item,
                                    [url + "/entity/" + column_type],
                                    "Step 5",
                                    list(set(WDtype.itemLabel.to_list())),
                                ]
                            )
                except Exception:
                    pass
    ...
    if step6:
        ...
        if col_type.get(0) and len(nomatch_row) > 0:
            for column_type in col_type.get(0):
                try:
                    WDtype_itemLabel_to_list = bbw_search_fn.get_SPARQL_dataframe_type2(
                        column_type, language
                    )
                    for row in nomatch_row or []:
                        proper_name = difflib.get_close_matches(
                            filecsv.iloc[row, 0],
                            WDtype_itemLabel_to_list,
                            n=15,
                            cutoff=0.95,
                        )
                        ...
                        if len(proper_name) > 0:
                            test_list = []
                            for proper in proper_name:
                                e = bbw_search_fn._original_get_SPARQL_dataframe(
                                    proper, language, extra="?itemLabel "
                                )
                                if isinstance(e, pd.DataFrame):
                                    test_list.append(e)
                            if len(test_list) > 0:
                            ...
                            if isinstance(WDdf, pd.DataFrame):
                                for col in range(col0, cols):
                                    try:
                                        ...
                                        properties = [
                                            x.replace(
                                                "/prop/P", "/prop/direct/P"
                                            ).replace("/direct-normalized/", "/direct/")
                                            for x in df_prop.p2.to_list()
                                        ]
                                        properties = list(
                                            set(zip(properties, df_prop.item.to_list()))
                        ...
                        if len(this_row_item) > 0:
                            ...
                            for i, cpa_row in enumerate(cpa_list[cpa_row_ind:]):
                                if len(cpa_row[4]) > 0:
                                    cpa_list[cpa_row_ind + i][4] = [
                                        prop
                                        for prop in cpa_row[4]
                                        if prop[1] == this_row_item
                                    ]
...
def postprocessing(
    ...
    bbw_cpa_sub = (
        bbw_cpa_few.groupby(["file", "column0", "column"])
        .agg({"property": lambda x: tuple(x)})
        .reset_index()
    )  # because a list is unhashable
    bbw_cpa_sub["property"] = bbw_cpa_sub["property"].apply(
        lambda x: [y[0] for subx in x for y in subx]
    )  # flatten
    bbw_cpa_sub["property"] = bbw_cpa_sub["property"].apply(
        lambda x: Counter(x).most_common(2)
    )
    bbw_cpa_sub["property"] = bbw_cpa_sub["property"].apply(
        lambda x: None if len(x) == 0 else x[0][0]
    )
    ...
    if filelist and isinstance(target_cpa, pd.DataFrame):
        bbw_cpa_sub = pd.merge(
            target_cpa[target_cpa.file.isin(filelist)].astype(
                {"file": str, "column0": int, "column": int}
            ),
            bbw_cpa_sub.astype(
                {"file": str, "column0": int, "column": int, "property": str}
            ),
            on=["file", "column0", "column"],
            how="inner",
        )
    # Create CEA-dataframe from the list and drop rows with None or empty lists
    bbw_few = pd.DataFrame(
        cea_list,
        columns=[
            "file",
            "row",
    ...
    bbw_cea_sub = (
        bbw_few.groupby(["file", "row", "column"])
        .agg({"item": lambda x: tuple(x)})
        .reset_index()
    )
    bbw_cea_sub["item"] = bbw_cea_sub["item"].apply(
        lambda x: [y for subx in x for y in subx]
    )
    ...
    if filelist and isinstance(target_cea, pd.DataFrame):
        bbw_cea_sub = pd.merge(
            target_cea[target_cea.file.isin(filelist)].astype(
                {"file": str, "row": int, "column": int}
            ),
    ...
    bbw_cta_one = (
        bbw_few.groupby(["file", "column"])
        .agg({"itemType": lambda x: tuple(x)})
        .reset_index()
    )
    bbw_cta_one["itemType"] = bbw_cta_one["itemType"].apply(
        lambda x: [y for subx in x for y in subx]
    )
    ...
    if filelist and isinstance(target_cta, pd.DataFrame):
        bbw_cta_sub = pd.merge(
            target_cta[target_cta.file.isin(filelist)].astype(
                {"file": str, "column": int}
            ),
    ...
    if filelist and not gui:
        ...
        try:
            print(
                "CEA",
                round(stat_cea_matched / stat_cea_target, 4),
                stat_cea_matched,
                stat_cea_target,
                stat_cea_target - stat_cea_matched,
                sep="\t",
            )
        except Exception:
            pass
        try:
            print(
                "CTA",
                round(stat_cta_matched / stat_cta_target, 4),
                stat_cta_matched,
                stat_cta_target,
                stat_cta_target - stat_cta_matched,
                sep="\t",
            )
        except Exception:
            pass
        try:
            print(
                "CPA",
                round(stat_cpa_matched / stat_cpa_target, 4),
                stat_cpa_matched,
                stat_cpa_target,
                stat_cpa_target - stat_cpa_matched,
                sep="\t",
            )
        except Exception:
            pass
...
def annotate(
    filecsv, filename="", language="", semtab=False, target_cpa=None, target_cta=None
):
    """
    Parameters
    ----------
    ...
    [cpa, cea, nomatch] = contextual_matching(
        filecsv,
        filename,
        language,
        semtab=semtab,
    ...
    [cpa_sub, cea_sub, cta_sub] = postprocessing(
        cpa, cea, [filename], target_cpa=target_cpa, target_cta=target_cta, gui=True
    )
    bbwtable = filecsv
    ...
    if not cea_sub.empty:
        for row in set(cea_sub.row.to_list()) or []:
            for column in set(cea_sub.column.to_list()) or []:
                _tmp = cea_sub.item[
                    (cea_sub.row == row) & (cea_sub.column == column)
                ].to_list()
                if len(_tmp) > 0:
                    link = _tmp[0]
                    if link:
    ...
    if len(urltable) > 0 and "property" in urltable:
        urltable.loc["datatype"] = [
            get_datatype(prop) for prop in urltable.loc["property", :].to_list()
        ]
        labeltable.loc["datatype"] = urltable.loc["datatype"].apply(
            lambda x: x.split("#")[-1] if x else ""
        )
        bbwtable.loc["datatype"] = (
            '<a target="_blank" href="'
            + urltable.loc["datatype"]
            + '">'
            + labeltable.loc["datatype"]
            + "</a>"
        )
